...
def create_folder_area(anatom_structure_path: str, root: str):
    """
    Рекурсивное создание директории для Области исследования
    :param anatom_structure_path: Путь до папки анатомической структуры, н.р.: ХСО - "МРТ//Голова//ХСО"
    :param root: root directory
    :return: None
    """
    new_path = root + '/' + anatom_structure_path.replace('//', '/')
    try:
...
def start_segmentation(prepared_document: str, root: str, path: str, key_for_area: list, anatom_structure_path: str):
    """
    ...
    :param anatom_structure_path: path to anatomical structure directory
    :return: None
    """
    compare_set_list = []
...
def create_folder_pathology(anatom_structure_path: str, root: str, key_name: str):
    """
    Создание директорий одноименных ключевым словам в папке патологии
    :param anatom_structure_path: path to directory
    :param root: root directory
    :param key_name: key word
    :return: None
    """
    new_path = root + '/' + anatom_structure_path.replace('//', '/') + '/' + key_name
    try:
        os.makedirs(new_path)
        os.mkdir(root + '/' + anatom_structure_path.replace('//', '/') + '/Прочее')
        os.mkdir(root + '/' + anatom_structure_path.replace('//', '/') + '/Trash')
    except FileExistsError:
        print("")
    else:
        pass
...
def continue_segmentation(prepared_document: str, root: str, key_words_for_remove: tuple, pathology_key: str,
    ...
    new_path = root + '/' + anatom_structure_path.replace('//', '/') + '/' + key_name
    counter = len(os.listdir(root + '/' + anatom_structure_path.replace('//', '/') + '/' + key_name))
    try:
        if ".docx" in prepared_document:
            ...
            if set(compare_set_list) == set(control_set):
                if counter < 20 or key_name == 'Прочее':
                    ...
                    text = f"{anatom_structure_path}//{key_name}\n{key_name}\n-report-text-below-"
                    ...
                    new_current_path = root + '/' + anatom_structure_path.replace('//',
...
def find_sentences_with_key_word(key_words_for_remove, conclusion, number_paragraphs):
    """
    ...
    for sentence_number in range(number_paragraphs):
        for key_word in key_words_for_remove:
...
def remove_segmented(prepared_document: str, root: str, anatom_structure_path: str, key_name: str, corn_path: str):
    """
    ...
    curr_path = root + '/' + corn_path.replace('//', '/')
    new_path = root + '/' + anatom_structure_path.replace('//', '/') + '/' + key_name
    trash = root + '/' + anatom_structure_path.replace('//', '/') + '/Trash'
...
def segment_else(prepared_document: str, root: str, key_words_for_remove: tuple, anatom_structure_path: str):
    """
    Выявляем патологии не попавшие в списко с ключевыми словами
    :param prepared_document:
    :param root:
    :param key_words_for_remove:
    :param anatom_structure_path:
    :return:
    """
    curr_path = root + '/' + anatom_structure_path.replace('//', '/')
    new_path = root + '/' + anatom_structure_path.replace('//', '/') + '/Прочее'
